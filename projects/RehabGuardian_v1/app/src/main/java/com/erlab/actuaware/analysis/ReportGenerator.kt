...
object ReportGenerator {
    ...
    fun generate(frames: List<FrameData>, session: Session? = null): Report {
        ...
        val riskLevel = riskLevel(avgRisk)
    ...
    private fun buildSuggestions(avgRisk: Float, issues: List<String>): List<String> {
        ...
        if (avgRisk > RiskThresholds.DANGER) s.add("建议停止当前运动，充分休息")
        if (avgRisk > RiskThresholds.WARNING) s.add("降低运动强度，注意膝关节保护")
