...
class HDF5RehabDataset(Dataset):
    ...
    def _fit_normalizer_from_hdf5(self, normalizer, h5_path, in_split):
        ...
        for name, arr in [
            ...
            ("grf_left", grf_l), ("grf_right", grf_r),
            ("com", com),
        ]:
            stats[f"{name}_mean"] = arr.mean(0).astype(np.float32)
            stats[f"{name}_std"]  = (arr.std(0) + eps).astype(np.float32)

        # markers_flat may contain NaN for missing marker channels;
        # use nanmean/nanstd for valid channels, fallback to 0/1 for all-NaN
        mk_mean = np.nanmean(mk, axis=0).astype(np.float32)
        mk_std  = (np.nanstd(mk, axis=0) + eps).astype(np.float32)
        bad = ~np.isfinite(mk_mean)
        mk_mean[bad] = 0.0
        mk_std[bad]  = 1.0
        stats["markers_flat_mean"] = mk_mean
        stats["markers_flat_std"]  = mk_std
