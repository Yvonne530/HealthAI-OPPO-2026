package com.rehabguardian.inference
...
class MNNInferenceEngine(private val context: Context) {
    ...
    private val poseBuffer = SlidingWindowBuffer<FloatArray>(STGCN_INPUT_T)
        ...
        try {
            ...
            stgcnSession = stgcnNet?.createSession(
                MNNNetInstance.Config().apply {
                    numThread = 4
                    forwardType = MNNNetInstance.Config.FORWARD_OPENCL
                }
            )
            ...
            val normJson = context.assets.open("norm_stats.json").bufferedReader().readText()
    ...
    private fun loadModel(filename: String): MNNNetInstance {
        ...
        return MNNNetInstance.createFromFile(tmpFile.absolutePath)
    ...
    private fun runSTGCN(poseSeq: FloatArray): FloatArray? {
        return try {
            val session = stgcnSession ?: return null
            val input = session.getInput("visual_seq")
            input?.setInputFloatData(poseSeq)
            session.run()
    ...
    private fun runFNO(bioSeq: FloatArray): FloatArray? {
        return try {
            val session = fnoSession ?: return null
            val input = session.getInput("bio_seq")
            input?.setInputFloatData(bioSeq)
            session.run()
    ...
    private fun runRisk(riskSeq: FloatArray): FloatArray? {
        return try {
            val session = riskSession ?: return null
            val input = session.getInput("risk_feat")
            input?.setInputFloatData(riskSeq)
            session.run()
    ...
    private fun buildBioFeatureWithDerivatives(jointAngles: FloatArray): FloatArray {
        val ns = normStats
        val out = FloatArray(72)

        if (ns != null) {
            for (i in 0..22) {
                out[i] = (jointAngles[i] - ns.jointAnglesMean[i]) / ns.jointAnglesStd[i]
            }
        } else {
            jointAngles.copyInto(out, 0)
        }

        if (prevJointAngles != null) {
            for (i in 0..22) {
                val vel = jointAngles[i] - prevJointAngles!![i]
                out[23 + i] = if (ns != null) vel / ns.jointAnglesStd[i] else vel
            }
        }
        ...
        if (prevVel != null && prevJointAngles != null) {
            for (i in 0..22) {
                ...
                out[46 + i] = if (ns != null) acc / ns.jointAnglesStd[i] else acc
            }
        }
