...
import com.rehabguardian.data.HistoryRecord
import com.rehabguardian.data.Session
...
class SessionManager(private val context: Context) {
    ...
    fun generateReport(session: Session): String {
        ...
        val advice = when {
            session.avgRisk < 0.35f -> "运动状态良好，继续保持当前运动强度。"
            session.avgRisk < 0.70f -> "存在轻微风险，建议适当降低运动强度，注意落地姿势。"
            else -> "检测到高风险动作，建议停止运动，咨询专业康复师。"
        }
        return """
            🧾 ACL 损伤风险分析报告
            ━━━━━━━━━━━━━━━━━━━━━━━━
